...
export default function Items() {
  ...
  // Subscribe to socket events scoped to the current user
  useEffect(() => {
    const user = getCurrentUser();
    if (user?.id) {
      socket.connect();
      socket.emit('user_join', { userId: parseInt(user.id), email: user.email, role: user.role });
      socket.on('item:created', () => load());
      socket.on('item:updated', () => load());
      socket.on('item:deleted', () => load());
    }
    return () => {
